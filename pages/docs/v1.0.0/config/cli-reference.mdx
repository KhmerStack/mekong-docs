...
| `mekong update` | Auto-update to the latest release (auto-retries with `sudo` on protected paths) |
| `mekong uninstall` | Remove the `mekong` binary, config directory, and PATH entry |
...
Downloads and installs the latest `mekong` release in place. On macOS and Linux, if the binary lives in a protected directory (e.g. `/usr/local/bin`), the update automatically retries with `sudo` — you may be prompted for your password.

```bash
mekong update
# ✔  Updated mekong from v1.6.14 → v1.6.15
```

On Windows, because the running `.exe` is locked, the binary swap is scheduled via a hidden PowerShell script that runs after `mekong` exits and then opens a new terminal automatically.

---

## `mekong uninstall`

Completely removes `mekong` from your machine:

- Deletes the `mekong` binary (auto-retries with `sudo` on macOS/Linux if needed)
- Removes the config directory `~/.mekong/`
- On Linux/macOS: removes the `PATH` export line added by the installer from your shell RC file
...
```
  ✔  Removed binary: /usr/local/bin/mekong
  ✔  Removed config: /Users/you/.mekong
  ✔  Removed PATH entry from ~/.zshrc
  ✔  mekong uninstalled successfully
```

On Windows the binary and `%LOCALAPPDATA%\Programs\mekong\` directory are removed and the user `PATH` entry is cleaned up automatically.
...
MAINTENANCE
  mekong update                    Update to latest release (auto-sudo on protected paths)
  mekong uninstall                 Remove binary, config, and PATH entry
  mekong version                   Show installed version
