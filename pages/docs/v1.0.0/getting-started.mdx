...
**Current CLI highlights (v1.6.15):**
...
- `mekong uninstall` — completely remove the CLI, config directory, and PATH entry
- Windows arm64 support
- VS Code extension with account/login panel

**New in v1.6.6+:**
- `mekong update` auto-retries with `sudo` on macOS/Linux when the binary is in a protected path (e.g. `/usr/local/bin`) — no more manual `sudo mekong update`
- `mekong uninstall` also auto-retries with `sudo` on protected paths
- Windows update schedules a binary swap via PowerShell after `mekong` exits — no Administrator needed
...
See the [Installation](/docs/installation) page for the installer scripts, upgrade flow, and raw SSH usage.
...
- [Deployment](/docs/deployments) — upload a zip and host it on Mekong
